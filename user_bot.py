...
async def user_bot_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        if not countries:
            await query.message.reply_text("❌ لا يمكن تشغيل الصيد! يرجى الانتقال إلى ‹ اضافه دوله › وتفعيل دولة واحدة على الأقل لبدء اقتناص الأرقام.")
            return
        ...
        # تشغيل وظيفة الفحص والضخ الدورية في الخلفية كل 5 ثوانٍ تلقائياً
        ...
        await query.message.reply_text(
            ...
            f"🔄 بدأ البوت الآن بالاتصال التلقائي بموقع الأرقام وسيبدأ بإنزال الأرقام في قناتك فور توفرها كل 5 ثوانٍ..."
        )
        
    elif data == "stop_hunting":
        context.user_data["is_hunting"] = False
        # إلغاء وإيقاف مهمة الخلفية الخاصة بهذا المستخدم
        current_jobs = context.job_queue.get_jobs_by_name(f"hunt_{user_id}")
        if current_jobs:
            for job in current_jobs:
                job.schedule_removal()
        await update.message.reply_text("🛑 تم إيقاف عملية صيد وضخ الأرقام التلقائية بنجاح.")
